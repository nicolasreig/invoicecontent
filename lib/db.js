...
async function initDb() {
  load()
  console.log('[DB] initDb: users in store:', store.users.length)

  const exists = store.users.find(u => u.email === 'nicolas@invoicecontent.com')
  if (!exists) {
    console.log('[DB] seeding admin user...')
    const id = nextId('users')
    const password_hash = await bcrypt.hash('N1c0las#123', 10)
    store.users.push({
      id,
      name: 'Nicolas',
    ...
    save()
    console.log('[DB] admin user seeded, store.users:', store.users.length)
  } else {
