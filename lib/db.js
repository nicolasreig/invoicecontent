...
async function initDb() {
  ...
  const existing = makeStatement('SELECT id FROM users WHERE email = ?').get('nicolas@invoicecontent.com')
  if (!existing) {
    const hash = await bcrypt.hash('N1c0las#123', 10)
    makeStatement('INSERT INTO users (name, email, password_hash) VALUES (?, ?, ?)').run('Nicolas', 'nicolas@invoicecontent.com', hash)
  }
